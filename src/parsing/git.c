...
static char *find_git(size_t *count)
{
    ...
    {
        *count = char_count(get_infos()->pwd, '/') - char_count(git_dir,'/') - 2;
        return (git_dir);
    }
    return (NULL);
}
...
char *draw_prompt(size_t count)
{
    ...
    if (!infos->git_branch || infos->git_branch[0] == '\0')
        new = ft_stringf(LBLUE"%s"YLW " ~ " GRY, infos->pwd);
    else
    {
		new = ft_stringf(LBLUE"%s"BLUE "git:" RED "(%s)" YLW "%s " GRY, infos->pwd, infos->git_branch, cursor);
    }
    return (new);
}